/*
    ----- RAK10701-P Field Tester for LoRaWAN Decoder Loriot Payload ------

    "Explanation: This code is for RAK10701-P Field Tester for LoRaWAN
    developed by Voxcom. It extracts the frame generated by the RAKWireless 
    hardware sent with Loriot LNS and decodes the data in Datacake into 
    a JSON string for general purposes."

    Copyright (C) 2023 Vox Comunicaciones S.A.
    http://www.voxcom.cl
...
function Decoder(bytes, fPort) {
    ...
    if (fPort === 1) {
        ...
        else if (typeof (rawPayload.gws) != "undefined" && Array.isArray(rawPayload.gws) && rawPayload.gws.length > 0) {
        // Check for presence of 'encdata' or 'data' in Loriot Payload
            if (typeof (rawPayload.encdata) != "undefined" || typeof (rawPayload.data) != "undefined") {
        ...
        for (idx = 0; idx < decoded.num_gw; idx++) {
            ...
            switch (server_type) {
                ...
                //Loriot
                case 4:
                    if (rawPayload.gws && rawPayload.gws.length > 0) {
                    gw_lat[idx] = rawPayload.gws[idx].lat;
                    gw_long[idx] = rawPayload.gws[idx].lon;
                    }       
                    else {
                        console.log("No se encontraron datos de latitud y longitud en el objeto gws.");
                    }
                    break;
